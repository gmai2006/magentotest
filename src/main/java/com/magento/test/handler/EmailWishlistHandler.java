...
// @Stateless
@Named("EmailWishlistHandler")
public class EmailWishlistHandler extends DelimiterFileHandler<EmailWishlist> {

  @Inject
  @Named("DefaultJpaDao")
  public EmailWishlistHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected EmailWishlist parseLine(List<String> headers, List<String> tokens) {
    EmailWishlist record = new EmailWishlist();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "wishlistImported":
          record.setWishlistImported(java.lang.Integer.valueOf((tokens.get(i))));
          break;
        case "wishlistModified":
          record.setWishlistModified(java.lang.Integer.valueOf((tokens.get(i))));
          break;
        case "createdAt":
