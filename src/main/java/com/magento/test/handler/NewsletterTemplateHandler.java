...
// @Stateless
@Named("NewsletterTemplateHandler")
public class NewsletterTemplateHandler extends DelimiterFileHandler<NewsletterTemplate> {

  @Inject
  @Named("DefaultJpaDao")
  public NewsletterTemplateHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected NewsletterTemplate parseLine(List<String> headers, List<String> tokens) {
    NewsletterTemplate record = new NewsletterTemplate();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "templateActual":
          record.setTemplateActual(java.lang.Integer.valueOf((tokens.get(i))));
          break;
        case "addedAt":
