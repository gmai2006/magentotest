...
// @Stateless
@Named("TemandoQuoteCollectionPointHandler")
public class TemandoQuoteCollectionPointHandler
    extends DelimiterFileHandler<TemandoQuoteCollectionPoint> {

  @Inject
  @Named("DefaultJpaDao")
  public TemandoQuoteCollectionPointHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected TemandoQuoteCollectionPoint parseLine(List<String> headers, List<String> tokens) {
    TemandoQuoteCollectionPoint record = new TemandoQuoteCollectionPoint();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "collectionPointId":
          record.setCollectionPointId(tokens.get(i));
          break;
