...
// @Stateless
@Named("OauthConsumerHandler")
public class OauthConsumerHandler extends DelimiterFileHandler<OauthConsumer> {

  @Inject
  @Named("DefaultJpaDao")
  public OauthConsumerHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected OauthConsumer parseLine(List<String> headers, List<String> tokens) {
    OauthConsumer record = new OauthConsumer();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "rejectedCallbackUrl":
          record.setRejectedCallbackUrl(tokens.get(i));
          break;
