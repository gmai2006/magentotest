...
// @Stateless
@Named("EmailReviewHandler")
public class EmailReviewHandler extends DelimiterFileHandler<EmailReview> {

  @Inject
  @Named("DefaultJpaDao")
  public EmailReviewHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected EmailReview parseLine(List<String> headers, List<String> tokens) {
    EmailReview record = new EmailReview();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "storeId":
          ...
          break;
        case "reviewImported":
          record.setReviewImported(java.lang.Integer.valueOf((tokens.get(i))));
          break;
        case "createdAt":
