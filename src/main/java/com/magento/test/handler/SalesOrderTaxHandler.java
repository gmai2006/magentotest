...
// @Stateless
@Named("SalesOrderTaxHandler")
public class SalesOrderTaxHandler extends DelimiterFileHandler<SalesOrderTax> {

  @Inject
  @Named("DefaultJpaDao")
  public SalesOrderTaxHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected SalesOrderTax parseLine(List<String> headers, List<String> tokens) {
    SalesOrderTax record = new SalesOrderTax();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "percent":
          record.setPercent(new java.math.BigDecimal(tokens.get(i)));
          break;
        ...
        case "process":
          record.setProcess(java.lang.Integer.valueOf((tokens.get(i))));
          break;
        case "baseRealAmount":
          record.setBaseRealAmount(new java.math.BigDecimal(tokens.get(i)));
          break;
