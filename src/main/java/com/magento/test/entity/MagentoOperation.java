/**
 * %% Copyright (C) 2021 DataScience 9 LLC %% Licensed under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the License. You may obtain a
 * copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License. #L%
 *
 * <p>This code is 100% AUTO generated. Please do not modify it DIRECTLY If you need new features or
 * function or changes please update the templates then submit the template through our web
 * interface.
 */
package com.magento.test.entity;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Table;
import java.io.Serializable;
import javax.persistence.Basic;

@Entity
@Table(name = "magento_operation")
public class MagentoOperation implements Serializable {
  private static final long serialVersionUID = 163445090566668857L;

  /** Description: id. */
  @javax.validation.constraints.NotNull
  @javax.persistence.Id
  @Column(name = "id")
  private java.lang.Integer id;

  /** Description: bulk_uuid. */
  @Basic
  @Column(name = "bulk_uuid")
  private byte[] bulkUuid;
  /** Description: topic_name. */
  @Basic
  @Column(name = "topic_name", length = 255)
  private java.lang.String topicName;
  /** Description: serialized_data. */
  @Basic
  @Column(name = "serialized_data")
  private java.lang.String serializedData;
  /** Description: result_serialized_data. */
  @Basic
  @Column(name = "result_serialized_data")
  private java.lang.String resultSerializedData;
  /** Description: status. */
  @Basic
  @Column(name = "status")
  private java.lang.Integer status;
  /** Description: error_code. */
  @Basic
  @Column(name = "error_code")
  private java.lang.Integer errorCode;
  /** Description: result_message. */
  @Basic
  @Column(name = "result_message", length = 255)
  private java.lang.String resultMessage;

  public MagentoOperation() {}

  public java.lang.Integer getId() {
    return this.id;
  }

  public void setId(java.lang.Integer id) {
    this.id = id;
  }

  public byte[] getBulkUuid() {
    return this.bulkUuid;
  }

  public java.lang.String getTopicName() {
    return this.topicName;
  }

  public java.lang.String getSerializedData() {
    return this.serializedData;
  }

  public java.lang.String getResultSerializedData() {
    return this.resultSerializedData;
  }

  public java.lang.Integer getStatus() {
    return this.status;
  }

  public java.lang.Integer getErrorCode() {
    return this.errorCode;
  }

  public java.lang.String getResultMessage() {
    return this.resultMessage;
  }

  public void setBulkUuid(byte[] bulkUuid) {
    this.bulkUuid = bulkUuid;
  }

  public void setTopicName(java.lang.String topicName) {
    this.topicName = topicName;
  }

  public void setSerializedData(java.lang.String serializedData) {
    this.serializedData = serializedData;
  }

  public void setResultSerializedData(java.lang.String resultSerializedData) {
    this.resultSerializedData = resultSerializedData;
  }

  public void setStatus(java.lang.Integer status) {
    this.status = status;
  }

  public void setErrorCode(java.lang.Integer errorCode) {
    this.errorCode = errorCode;
  }

  public void setResultMessage(java.lang.String resultMessage) {
    this.resultMessage = resultMessage;
  }
}
