...
import com.magento.test.dao.DefaultVertexTaxrequestDao;
...
public class VertexTaxrequestHandlerTestIt {
  static final String inputFile = "VertexTaxrequest.json";
  static VertexTaxrequestHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    VertexTaxrequest testResult = jpa.find(VertexTaxrequest.class, records[0].getRequestId());
    assertEquals(
        "expect equals requestType ",
        this.records[0].getRequestType(),
        testResult.getRequestType());
    assertEquals(
        "expect equals totalTax ", this.records[0].getTotalTax(), testResult.getTotalTax());
    assertEquals(
        "expect equals sourcePath ", this.records[0].getSourcePath(), testResult.getSourcePath());
    assertEquals(
        "expect equals taxAreaId ", this.records[0].getTaxAreaId(), testResult.getTaxAreaId());
    assertEquals(
    ...
    assertEquals(
        ...
        this.records[0].getLookupResult(),
        testResult.getLookupResult());
    assertEquals(
        "expect equals requestXml ", this.records[0].getRequestXml(), testResult.getRequestXml());
    assertEquals(
        "expect equals responseXml ",
        this.records[0].getResponseXml(),
