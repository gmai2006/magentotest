...
import com.magento.test.dao.DefaultSalesInvoicedAggregatedDao;
...
public class SalesInvoicedAggregatedHandlerTestIt {
  static final String inputFile = "SalesInvoicedAggregated.json";
  static SalesInvoicedAggregatedHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private SalesInvoicedAggregated[] records;
  ...
  public void testSelect() throws IOException {
    ...
    records = gson.fromJson(json, SalesInvoicedAggregated[].class);
    assertEquals("match count", count, records.length);
    final SalesInvoicedAggregatedId id =
        new SalesInvoicedAggregatedId(
            this.records[0].getStoreId(),
            this.records[0].getOrderStatus(),
            this.records[0].getPeriod(),
            this.records[0].getId());
    SalesInvoicedAggregated testResult = jpa.find(SalesInvoicedAggregated.class, id);
    ...
    assertEquals(
        ...
        this.records[0].getOrdersInvoiced(),
        testResult.getOrdersInvoiced());
    assertEquals(
        "expect equals invoiced ", this.records[0].getInvoiced(), testResult.getInvoiced());
    assertEquals(
        "expect equals invoicedCaptured ",
        this.records[0].getInvoicedCaptured(),
        testResult.getInvoicedCaptured());
    assertEquals(
        "expect equals invoicedNotCaptured ",
        this.records[0].getInvoicedNotCaptured(),
