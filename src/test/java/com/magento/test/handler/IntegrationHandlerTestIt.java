...
import com.magento.test.dao.DefaultIntegrationDao;
...
public class IntegrationHandlerTestIt {
  static final String inputFile = "Integration.json";
  static IntegrationHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    final IntegrationId id =
        new IntegrationId(
            this.records[0].getConsumerId(),
            this.records[0].getIntegrationId(),
            this.records[0].getName());
    Integration testResult = jpa.find(Integration.class, id);
    assertEquals("expect equals email ", this.records[0].getEmail(), testResult.getEmail());
    assertEquals(
        "expect equals endpoint ", this.records[0].getEndpoint(), testResult.getEndpoint());
    org.junit.Assert.assertEquals(
        "expect equals status ", this.records[0].getStatus(), testResult.getStatus());
    org.junit.Assert.assertEquals(
        "expect equals setupType ", this.records[0].getSetupType(), testResult.getSetupType());
    assertEquals(
        "expect equals identityLinkUrl ",
        this.records[0].getIdentityLinkUrl(),
