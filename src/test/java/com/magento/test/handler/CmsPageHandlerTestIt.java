...
import com.magento.test.dao.DefaultCmsPageDao;
...
public class CmsPageHandlerTestIt {
  static final String inputFile = "CmsPage.json";
  static CmsPageHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    CmsPage testResult = jpa.find(CmsPage.class, records[0].getPageId());
    assertEquals("expect equals title ", this.records[0].getTitle(), testResult.getTitle());
    assertEquals(
        "expect equals pageLayout ", this.records[0].getPageLayout(), testResult.getPageLayout());
    assertEquals(
        "expect equals metaKeywords ",
        this.records[0].getMetaKeywords(),
        testResult.getMetaKeywords());
    assertEquals(
        "expect equals metaDescription ",
        this.records[0].getMetaDescription(),
        testResult.getMetaDescription());
    assertEquals(
        "expect equals identifier ", this.records[0].getIdentifier(), testResult.getIdentifier());
    assertEquals(
        "expect equals contentHeading ",
        this.records[0].getContentHeading(),
        testResult.getContentHeading());
    assertEquals("expect equals content ", this.records[0].getContent(), testResult.getContent());
