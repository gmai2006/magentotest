...
import com.magento.test.dao.DefaultKlarnaPaymentsQuoteDao;
...
public class KlarnaPaymentsQuoteHandlerTestIt {
  static final String inputFile = "KlarnaPaymentsQuote.json";
  static KlarnaPaymentsQuoteHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private KlarnaPaymentsQuote[] records;
  ...
  public void testSelect() throws IOException {
    ...
    KlarnaPaymentsQuote testResult =
        jpa.find(KlarnaPaymentsQuote.class, records[0].getPaymentsQuoteId());
    assertEquals(
        "expect equals sessionId ", this.records[0].getSessionId(), testResult.getSessionId());
    assertEquals(
        "expect equals clientToken ",
        this.records[0].getClientToken(),
        testResult.getClientToken());
    assertEquals(
        "expect equals authorizationToken ",
        this.records[0].getAuthorizationToken(),
    ...
    assertEquals(
        ...
        this.records[0].getPaymentMethods(),
        testResult.getPaymentMethods());
    assertEquals(
        "expect equals paymentMethodInfo ",
        this.records[0].getPaymentMethodInfo(),
