...
import com.magento.test.dao.DefaultSearchQueryDao;
...
public class SearchQueryHandlerTestIt {
  static final String inputFile = "SearchQuery.json";
  static SearchQueryHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    final SearchQueryId id =
        new SearchQueryId(
            ...
            this.records[0].getQueryId(),
            this.records[0].getQueryText());
    SearchQuery testResult = jpa.find(SearchQuery.class, id);
    org.junit.Assert.assertEquals(
        "expect equals numResults ", this.records[0].getNumResults(), testResult.getNumResults());
    org.junit.Assert.assertEquals(
        "expect equals popularity ", this.records[0].getPopularity(), testResult.getPopularity());
    assertEquals(
        "expect equals redirect ", this.records[0].getRedirect(), testResult.getRedirect());
    org.junit.Assert.assertEquals(
        "expect equals displayInTerms ",
        this.records[0].getDisplayInTerms(),
        testResult.getDisplayInTerms());
    org.junit.Assert.assertEquals(
        "expect equals isActive ", this.records[0].getIsActive(), testResult.getIsActive());
    org.junit.Assert.assertEquals(
        "expect equals isProcessed ",
        this.records[0].getIsProcessed(),
        testResult.getIsProcessed());
