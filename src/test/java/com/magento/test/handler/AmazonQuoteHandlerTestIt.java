...
import com.magento.test.dao.DefaultAmazonQuoteDao;
...
public class AmazonQuoteHandlerTestIt {
  static final String inputFile = "AmazonQuote.json";
  static AmazonQuoteHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    final AmazonQuoteId id =
        new AmazonQuoteId(this.records[0].getQuoteId(), this.records[0].getEntityId());
    AmazonQuote testResult = jpa.find(AmazonQuote.class, id);
    assertEquals(
        "expect equals amazonOrderReferenceId ",
        this.records[0].getAmazonOrderReferenceId(),
        testResult.getAmazonOrderReferenceId());
    assertEquals(
        "expect equals sandboxSimulationReference ",
        this.records[0].getSandboxSimulationReference(),
        testResult.getSandboxSimulationReference());
    org.junit.Assert.assertEquals(
        "expect equals confirmed ", this.records[0].getConfirmed(), testResult.getConfirmed());
