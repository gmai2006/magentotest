...
import com.magento.test.dao.DefaultSalesOrderStatusHistoryDao;
...
public class SalesOrderStatusHistoryHandlerTestIt {
  static final String inputFile = "SalesOrderStatusHistory.json";
  static SalesOrderStatusHistoryHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private SalesOrderStatusHistory[] records;
  ...
  public void testSelect() throws IOException {
    ...
    records = gson.fromJson(json, SalesOrderStatusHistory[].class);
    assertEquals("match count", count, records.length);
    SalesOrderStatusHistory testResult =
        jpa.find(SalesOrderStatusHistory.class, records[0].getEntityId());
    org.junit.Assert.assertEquals(
        "expect equals parentId ", this.records[0].getParentId(), testResult.getParentId());
    org.junit.Assert.assertEquals(
        "expect equals isCustomerNotified ",
        this.records[0].getIsCustomerNotified(),
        testResult.getIsCustomerNotified());
    org.junit.Assert.assertEquals(
    ...
    assertEquals(
        "expect equals entityName ", this.records[0].getEntityName(), testResult.getEntityName());
