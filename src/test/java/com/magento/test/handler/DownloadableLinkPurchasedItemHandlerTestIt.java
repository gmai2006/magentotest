...
import com.magento.test.dao.DefaultDownloadableLinkPurchasedItemDao;
...
public class DownloadableLinkPurchasedItemHandlerTestIt {
  static final String inputFile = "DownloadableLinkPurchasedItem.json";
  static DownloadableLinkPurchasedItemHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private DownloadableLinkPurchasedItem[] records;
  ...
  public void testSelect() throws IOException {
    ...
    DownloadableLinkPurchasedItem testResult =
        jpa.find(DownloadableLinkPurchasedItem.class, records[0].getItemId());
    org.junit.Assert.assertEquals(
        "expect equals purchasedId ",
        this.records[0].getPurchasedId(),
        testResult.getPurchasedId());
    org.junit.Assert.assertEquals(
    ...
    assertEquals(
        "expect equals linkHash ", this.records[0].getLinkHash(), testResult.getLinkHash());
    org.junit.Assert.assertEquals(
        "expect equals numberOfDownloadsBought ",
        this.records[0].getNumberOfDownloadsBought(),
        testResult.getNumberOfDownloadsBought());
    org.junit.Assert.assertEquals(
        "expect equals numberOfDownloadsUsed ",
        this.records[0].getNumberOfDownloadsUsed(),
        testResult.getNumberOfDownloadsUsed());
    org.junit.Assert.assertEquals(
        "expect equals linkId ", this.records[0].getLinkId(), testResult.getLinkId());
    assertEquals(
