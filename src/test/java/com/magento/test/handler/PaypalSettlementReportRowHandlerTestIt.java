...
import com.magento.test.dao.DefaultPaypalSettlementReportRowDao;
...
public class PaypalSettlementReportRowHandlerTestIt {
  static final String inputFile = "PaypalSettlementReportRow.json";
  static PaypalSettlementReportRowHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    PaypalSettlementReportRow testResult =
        jpa.find(PaypalSettlementReportRow.class, records[0].getRowId());
    org.junit.Assert.assertEquals(
        "expect equals reportId ", this.records[0].getReportId(), testResult.getReportId());
    assertEquals(
        "expect equals transactionId ",
        this.records[0].getTransactionId(),
        testResult.getTransactionId());
    assertEquals(
        "expect equals invoiceId ", this.records[0].getInvoiceId(), testResult.getInvoiceId());
    assertEquals(
        "expect equals paypalReferenceId ",
        this.records[0].getPaypalReferenceId(),
        testResult.getPaypalReferenceId());
    assertEquals(
        "expect equals paypalReferenceIdType ",
        this.records[0].getPaypalReferenceIdType(),
        testResult.getPaypalReferenceIdType());
    assertEquals(
        "expect equals transactionEventCode ",
        this.records[0].getTransactionEventCode(),
        testResult.getTransactionEventCode());
    assertEquals(
        "expect equals transactionDebitOrCredit ",
        this.records[0].getTransactionDebitOrCredit(),
        testResult.getTransactionDebitOrCredit());
    assertEquals(
        "expect equals grossTransactionAmount ",
        this.records[0].getGrossTransactionAmount(),
        testResult.getGrossTransactionAmount());
    assertEquals(
        "expect equals grossTransactionCurrency ",
        this.records[0].getGrossTransactionCurrency(),
        testResult.getGrossTransactionCurrency());
    assertEquals(
        "expect equals feeDebitOrCredit ",
        this.records[0].getFeeDebitOrCredit(),
        testResult.getFeeDebitOrCredit());
    assertEquals(
        "expect equals feeAmount ", this.records[0].getFeeAmount(), testResult.getFeeAmount());
    assertEquals(
        "expect equals feeCurrency ",
        this.records[0].getFeeCurrency(),
    ...
    assertEquals(
        ...
        this.records[0].getPaymentTrackingId(),
        testResult.getPaymentTrackingId());
    assertEquals("expect equals storeId ", this.records[0].getStoreId(), testResult.getStoreId());
