...
import com.magento.test.dao.DefaultNewsletterQueueDao;
...
public class NewsletterQueueHandlerTestIt {
  static final String inputFile = "NewsletterQueue.json";
  static NewsletterQueueHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    NewsletterQueue testResult = jpa.find(NewsletterQueue.class, records[0].getQueueId());
    org.junit.Assert.assertEquals(
        "expect equals templateId ", this.records[0].getTemplateId(), testResult.getTemplateId());
    org.junit.Assert.assertEquals(
        "expect equals newsletterType ",
        this.records[0].getNewsletterType(),
        testResult.getNewsletterType());
    assertEquals(
        "expect equals newsletterText ",
        this.records[0].getNewsletterText(),
        testResult.getNewsletterText());
    assertEquals(
        "expect equals newsletterStyles ",
        this.records[0].getNewsletterStyles(),
        testResult.getNewsletterStyles());
    assertEquals(
        "expect equals newsletterSubject ",
        this.records[0].getNewsletterSubject(),
        testResult.getNewsletterSubject());
    assertEquals(
        "expect equals newsletterSenderName ",
        this.records[0].getNewsletterSenderName(),
        testResult.getNewsletterSenderName());
    assertEquals(
        "expect equals newsletterSenderEmail ",
        this.records[0].getNewsletterSenderEmail(),
        testResult.getNewsletterSenderEmail());
    org.junit.Assert.assertEquals(
        "expect equals queueStatus ",
        this.records[0].getQueueStatus(),
        testResult.getQueueStatus());
