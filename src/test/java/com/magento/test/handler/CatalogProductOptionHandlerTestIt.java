...
import com.magento.test.dao.DefaultCatalogProductOptionDao;
...
public class CatalogProductOptionHandlerTestIt {
  static final String inputFile = "CatalogProductOption.json";
  static CatalogProductOptionHandler handler;
  ...
  static Gson gson =
      new GsonBuilder()
          ...
          .create();
  private CatalogProductOption[] records;
  ...
  public void testSelect() throws IOException {
    ...
    CatalogProductOption testResult =
        jpa.find(CatalogProductOption.class, records[0].getOptionId());
    ...
    org.junit.Assert.assertEquals(
        "expect equals isRequire ", this.records[0].getIsRequire(), testResult.getIsRequire());
    assertEquals("expect equals sku ", this.records[0].getSku(), testResult.getSku());
    org.junit.Assert.assertEquals(
        "expect equals maxCharacters ",
        this.records[0].getMaxCharacters(),
        testResult.getMaxCharacters());
    assertEquals(
        "expect equals fileExtension ",
        this.records[0].getFileExtension(),
        testResult.getFileExtension());
    org.junit.Assert.assertEquals(
        "expect equals imageSizex ", this.records[0].getImageSizex(), testResult.getImageSizex());
    org.junit.Assert.assertEquals(
        "expect equals imageSizey ", this.records[0].getImageSizey(), testResult.getImageSizey());
    org.junit.Assert.assertEquals(
