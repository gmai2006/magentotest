...
import com.magento.test.dao.DefaultEmailContactDao;
...
public class EmailContactHandlerTestIt {
  static final String inputFile = "EmailContact.json";
  static EmailContactHandler handler;
  ...
  public void testSelect() throws IOException {
    ...
    EmailContact testResult = jpa.find(EmailContact.class, records[0].getEmailContactId());
    org.junit.Assert.assertEquals(
        "expect equals isGuest ", this.records[0].getIsGuest(), testResult.getIsGuest());
    assertEquals(
        "expect equals contactId ", this.records[0].getContactId(), testResult.getContactId());
    org.junit.Assert.assertEquals(
    ...
    org.junit.Assert.assertEquals(
        ...
        this.records[0].getIsSubscriber(),
        testResult.getIsSubscriber());
    org.junit.Assert.assertEquals(
        "expect equals subscriberStatus ",
        this.records[0].getSubscriberStatus(),
        testResult.getSubscriberStatus());
    org.junit.Assert.assertEquals(
        "expect equals emailImported ",
        this.records[0].getEmailImported(),
        testResult.getEmailImported());
    org.junit.Assert.assertEquals(
        "expect equals subscriberImported ",
        this.records[0].getSubscriberImported(),
        testResult.getSubscriberImported());
    org.junit.Assert.assertEquals(
        "expect equals suppressed ", this.records[0].getSuppressed(), testResult.getSuppressed());
