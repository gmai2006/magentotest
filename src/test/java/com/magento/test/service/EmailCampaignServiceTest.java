...
public class EmailCampaignServiceTest {
  private static DefaultEmailCampaignService serviceMock;
  private static EmailCampaign[] records;
  static Gson gson =
      new GsonBuilder()
  ...
  public static void setUp() {
    serviceMock = mock(DefaultEmailCampaignService.class);
    String inputFile = "EmailCampaign.json";
    try {
  ...
  public void testFind_success() {
    EmailCampaign testResult = serviceMock.find(records[0].getId());

    org.junit.Assert.assertNotNull(testResult);
    org.junit.Assert.assertTrue(
        "expect equals campaignId ", records[0].getCampaignId() == testResult.getCampaignId());
    ...
    org.junit.Assert.assertEquals(
        "expect equals eventName ", records[0].getEventName(), testResult.getEventName());
    org.junit.Assert.assertEquals(
        "expect equals sendId ", records[0].getSendId(), testResult.getSendId());
    org.junit.Assert.assertTrue(
        "expect equals sendStatus ", records[0].getSendStatus() == testResult.getSendStatus());
  }
}
