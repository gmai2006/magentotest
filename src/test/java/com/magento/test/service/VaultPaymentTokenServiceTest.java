...
public class VaultPaymentTokenServiceTest {
  private static DefaultVaultPaymentTokenService serviceMock;
  private static VaultPaymentToken[] records;
  static Gson gson =
      new GsonBuilder()
  ...
  public static void setUp() {
    serviceMock = mock(DefaultVaultPaymentTokenService.class);
    String inputFile = "VaultPaymentToken.json";
    try {
    ...
    final VaultPaymentTokenId id =
        new VaultPaymentTokenId(
            records[0].getPaymentMethodCode(),
            records[0].getGatewayToken(),
            records[0].getEntityId(),
            records[0].getCustomerId(),
            records[0].getPublicHash());
  ...
  public void testFind_success() {
    final VaultPaymentTokenId id =
        new VaultPaymentTokenId(
            ...
            records[0].getCustomerId(),
            records[0].getPublicHash());
    VaultPaymentToken testResult = serviceMock.find(id);
    org.junit.Assert.assertNotNull(testResult);
    org.junit.Assert.assertEquals(
        "expect equals type ", records[0].getType(), testResult.getType());
    org.junit.Assert.assertEquals(
        "expect equals details ", records[0].getDetails(), testResult.getDetails());
