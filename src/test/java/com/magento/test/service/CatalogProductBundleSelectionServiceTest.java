...
public class CatalogProductBundleSelectionServiceTest {
  private static DefaultCatalogProductBundleSelectionService serviceMock;
  private static CatalogProductBundleSelection[] records;
  static Gson gson =
      new GsonBuilder()
  ...
  public static void setUp() {
    serviceMock = mock(DefaultCatalogProductBundleSelectionService.class);
    String inputFile = "CatalogProductBundleSelection.json";
    try {
    ...
    // test data
    when(serviceMock.find(records[0].getSelectionId())).thenReturn(records[0]);
  }

  @Test
  public void testFind_success() {
    CatalogProductBundleSelection testResult = serviceMock.find(records[0].getSelectionId());
    ...
    org.junit.Assert.assertEquals(
        "expect equals selectionQty ", records[0].getSelectionQty(), testResult.getSelectionQty());
    org.junit.Assert.assertTrue(
        "expect equals selectionCanChangeQty ",
        records[0].getSelectionCanChangeQty() == testResult.getSelectionCanChangeQty());
  }
}
