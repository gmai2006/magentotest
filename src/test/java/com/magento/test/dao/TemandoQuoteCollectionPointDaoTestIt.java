...
public class TemandoQuoteCollectionPointDaoTestIt {
  static final String inputFile = "TemandoQuoteCollectionPoint.json";
  static TemandoQuoteCollectionPointDao dao;
  static Gson gson =
      new GsonBuilder()
          .registerTypeHierarchyAdapter(byte[].class, new ByteArrayToBase64TypeAdapter())
          .setDateFormat("yyyy-MM-dd HH:mm:ss.S")
          .create();
  private TemandoQuoteCollectionPoint[] records;
  ...
  public static void beforeClass() {
    ...
    dao = new DefaultTemandoQuoteCollectionPointDao(jpa);
  }
  ...
  public void testSelect() {
    TemandoQuoteCollectionPoint testResult = dao.find(records[1].getEntityId());
    assertNotNull("expect result", testResult);
    org.junit.Assert.assertTrue(
        "expect equals recipientAddressId ",
        this.records[1].getRecipientAddressId() == testResult.getRecipientAddressId());
    ...
    org.junit.Assert.assertEquals(
        ...
        this.records[1].getOpeningHours(),
        testResult.getOpeningHours());
    org.junit.Assert.assertEquals(
        "expect equals shippingExperiences ",
        this.records[1].getShippingExperiences(),
        testResult.getShippingExperiences());
    org.junit.Assert.assertTrue(
        "expect equals selected ", this.records[1].getSelected() == testResult.getSelected());
    org.junit.Assert.assertTrue(
        "expect equals distance ", this.records[1].getDistance() == testResult.getDistance());
  }
}
